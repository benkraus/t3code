...
describe("orchestration projector", () => {
  ...
  it.effect("fails when event payload cannot be decoded by runtime schema", () =>
    Effect.gen(function* () {
      ...
      const error = yield* projectEvent(
        model,
        makeEvent({
          sequence: 1,
          type: "thread.created",
  ...
  it.effect("removes a settled turn checkpoint when HerdR reopens the turn", () =>
    Effect.gen(function* () {
      const createdAt = "2026-02-23T08:00:00.000Z";
      const turnId = "turn-1";
      const events: ReadonlyArray<OrchestrationEvent> = [
        makeEvent({
          ...
          payload: {
            ...
            projectId: "project-1",
            title: "demo",
            modelSelection: { provider: ProviderDriverKind.make("herdr"), model: "herdr-managed" },
            runtimeMode: "full-access",
            branch: null,
            worktreePath: null,
            createdAt,
        ...
        makeEvent({
          ...
          payload: {
            ...
            session: {
              ...
              activeTurnId: turnId,
              lastError: null,
              updatedAt: "2026-02-23T08:00:05.000Z",
            },
          },
        }),
        makeEvent({
          ...
          aggregateKind: "thread",
          aggregateId: "thread-1",
          occurredAt: "2026-02-23T08:01:00.000Z",
        ...
        makeEvent({
          ...
          aggregateKind: "thread",
          aggregateId: "thread-1",
          occurredAt: "2026-02-23T08:01:02.000Z",
          commandId: "cmd-ready",
          payload: {
            threadId: "thread-1",
            session: {
              ...
              activeTurnId: null,
              lastError: null,
              updatedAt: "2026-02-23T08:01:02.000Z",
            },
          },
        }),
        makeEvent({
          ...
          aggregateKind: "thread",
          aggregateId: "thread-1",
          occurredAt: "2026-02-23T08:01:05.000Z",
          ...
          payload: {
            ...
            session: {
              ...
              activeTurnId: turnId,
              lastError: null,
              updatedAt: "2026-02-23T08:01:05.000Z",
            },
            turnTiming: {
              turnId,
              startedAt: "2026-02-23T08:00:05.000Z",
            },
          },
        }),
      ];
      ...
      expect(projected.threads[0]?.latestTurn?.state).toBe("running");
      expect(projected.threads[0]?.checkpoints).toEqual([]);
    }),
  );
  ...
  it.effect("preserves terminal lifecycle timing for a later assistant snapshot update", () =>
    Effect.gen(function* () {
      const createdAt = "2026-02-23T09:00:00.000Z";
      const settledAt = "2026-02-23T09:01:00.000Z";
      const events: ReadonlyArray<OrchestrationEvent> = [
        makeEvent({
          sequence: 1,
          type: "thread.created",
