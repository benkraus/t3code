...
function maxIso(left: string | null, right: string): string {
  ...
  return maxIsoDateTime(left, right);
}
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                archiveCommandId: row.archiveCommandId ?? null,
                deletedAt: row.deletedAt,
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (
    ...
    Effect.gen(function* () {
      const [threadRow, latestTurnRow, sessionRow] = yield* Effect.all([
        (options?.includeArchived ? getNonDeletedThreadRowById : getActiveThreadRowById)({
          threadId,
        }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
      ...
      return Option.some({
        ...
        archiveCommandId: threadRow.value.archiveCommandId ?? null,
