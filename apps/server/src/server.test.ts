...
import { shouldRedirectHttpRequestToDevServer } from "./http.ts";
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves discovery info for remote host scans", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();

      const response = yield* HttpClient.get("/api/server-discovery");
      assert.equal(response.status, 200);
  ...
  it.effect("reports websocket auth requirements in discovery info", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest({
  ...
  it("only redirects loopback HTTP requests to the dev server", () => {
    assert.equal(shouldRedirectHttpRequestToDevServer(undefined), true);
    assert.equal(shouldRedirectHttpRequestToDevServer("127.0.0.1"), true);
    assert.equal(shouldRedirectHttpRequestToDevServer("::1"), true);
    assert.equal(shouldRedirectHttpRequestToDevServer("::ffff:127.0.0.1"), true);
    assert.equal(shouldRedirectHttpRequestToDevServer("192.168.1.20"), false);
    assert.equal(shouldRedirectHttpRequestToDevServer("::ffff:192.168.1.20"), false);
  });
